...
Message ItemAction::exportData(Message *msg) {
    ...
    XlsxUtil xls;
    QString arr;
    xls.write("VERSION " + msg->data("version").toString());
    ...
    arr.append("###CATEGORY\n");
    xls.write("###CATEGORY");
    xls.write(QVariantList{"id", "name", "code", "parent_id", "hierarchy"});
    ...
    for (int i = 0; i < cats.size(); i++) {
        ...
        xls.write(QVariantList{d["id"], d["name"].toString().trimmed().replace("\n", ""),
                               d["code"].toString().trimmed().replace("\n", ""),
                               QString::number(d["parent_id"].toInt()), d["hierarchy"].toString().trimmed()});
    }
    DbResult sups = mDb->table("supliers")->where("deleted_at IS NULL")->exec();
    xls.write("###SUPPLIER");
    xls.write(QVariantList{"id", "name", "code", "address", "phone", "email"});
    arr.append("###SUPPLIER\n");
    ...
    for (int i = 0; i < sups.size(); i++) {
        ...
        xls.write(QVariantList{d["id"], d["name"].toString().trimmed().replace("\n", ""),
                               d["code"].toString().trimmed().replace("\n", ""),
                               d["address"].toString().trimmed().replace("\n", ""),
                               d["phone"].toString().trimmed().replace("\n", ""), d["email"].toString().trimmed()});
    }
    xls.write("###ITEM");
    xls.write(QVariantList{"barcode",        "name",      "category",     "suplier",    "stock",
                           "buy_price",      "count1",    "sellprice1",   "discform1",  "count2",
                           "sellprice2",     "discform2", "count3",       "sellprice3", "discform3",
                           "calculatestock", "sellable",  "purchaseable", "box",        "multiprice",
                           "priceeditable",  "unit"});
    arr.append("###ITEM\n");
    arr.append("barcode;name;category;suplier;stock;buy_price;count1;sellprice1;"
               "discform1;count2;sellprice2;discform2;count3;sellprice3;"
               "discform3;calculatestock;sellable;purchaseable;box;multiprice;"
               "priceeditable;unit;\n");
    ...
    while (true) {
        ...
        for (int i = 0; i < res.size(); i++) {
            ...
            arr.append("\n");
            // qDebug() << d;
            xls.write(QVariantList{d["barcode"].toString().trimmed().replace("\n", ""),
                                   d["name"].toString().trimmed().replace("\n", ""),
                                   d["category"].toString().trimmed().replace("\n", ""),
                                   d["suplier"].toString().trimmed().replace("\n", ""),
                                   d["stock"].toString(),
                                   d["buy_price"].toString(),
                                   d["count1"].toString(),
                                   d["price1"].toString(),
                                   d["discform1"].toString(),
                                   d["count2"].toString(),
                                   d["price2"].toString(),
                                   d["discform2"].toString(),
                                   d["count3"].toString(),
                                   d["price3"].toString(),
                                   d["discform3"].toString(),
                                   (flag & ITEM_FLAG::CALCULATE_STOCK) == 0 ? "0" : "1",
                                   (flag & ITEM_FLAG::SELLABLE) == 0 ? "0" : "1",
                                   (flag & ITEM_FLAG::PURCHASE) == 0 ? "0" : "1",
                                   (flag & ITEM_FLAG::PACKAGE) == 0 ? "0" : "1",
                                   (flag & ITEM_FLAG::MULTIPRICE) == 0 ? "0" : "1",
                                   (flag & ITEM_FLAG::EDITABLE_PRICE) == 0 ? "0" : "1",
                                   d["unit"].toString().trimmed()});
        }
        start += limit;
    }
    // handle the link box here
    xls.write("###LINK");
    xls.write(QVariantList{"barcode", "type", "barcode_link", "count_link"});
    arr.append("###LINK\n");
    ...
    for (int i = 0; i < linkres.size(); i++) {
        ...
        xls.write(QVariantList{d["barcode"].toString().trimmed(), d["type"].toString(),
                               d["barcode_link"].toString().trimmed(), d["count_link"].toString()});
    }
    message.addData("data", arr);
    xls.save("/home/apin/sultan.xlsx");
    return message;
}
